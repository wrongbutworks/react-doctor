...
export const HANDLER_SNIPPET_POOL = [
  ...
  `const handleRefresh = async () => { setLoading(true); const [first, second] = await Promise.all([api.get(url).catch(() => null), fetch(url).then((response) => response.json()).catch(() => [])]); setState(first ?? second); setLoading(false); };`,
